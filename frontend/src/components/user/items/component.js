...
import * as ProfileActions from '../../../actions/user';
...
class ItemReviewComponent extends Component {
  ...
  getReview(page, ordering) {
    this.props.getReview(this.props.userId, page, ordering);
  ...
  handleScroll(event) {
    ...
    if (scrollTop + window.innerHeight > (0.9 * scrollHeight)) {
      if (!this.props.isFetchingReview && !this.props.noMoreReview) {
        this.getReview(this.props.userId, this.state.currentPage + 1,
